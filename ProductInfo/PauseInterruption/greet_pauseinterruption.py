...
def execute_greet_pauseinterruption():
    """Execute pause interruption sequence with two-part audio"""
    ...
    import os
    audio_file_6am = os.path.join(os.path.dirname(__file__), "6AM.mp3")
    audio_file_continues = os.path.join(os.path.dirname(__file__), "mistycontinues.mp3")
    
    if os.path.exists(audio_file_6am) and os.path.exists(audio_file_continues):
        print(f"Found both audio files: 6AM.mp3 and mistycontinues.mp3")
        
        # Upload both audio files using the proven method (False, True parameters)
        misty.save_audio("pause_6am.mp3", convert_to_byte(audio_file_6am), False, True)
        misty.save_audio("pause_continues.mp3", convert_to_byte(audio_file_continues), False, True)
        time.sleep(1)  # Wait for uploads
        
        # Part 1: "At 6 am!" - turns and looks at collaborator + raises arms + joy eyes
        ...
        # Play first part
        feedback1 = misty.play_audio(fileName="pause_6am.mp3", volume=80)
        time.sleep(2)  # Time for "At 6 am!"
        ...
        feedback2 = misty.play_audio(fileName="pause_continues.mp3", volume=80)
        ...
        return True, "✨ Pause interruption sequence completed"
    else:
        return False, f"❌ Audio files not found. Need both 6AM.mp3 and mistycontinues.mp3"
