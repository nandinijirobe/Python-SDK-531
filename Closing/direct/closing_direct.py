...
def execute_closing_direct():
    """Execute closing direct sequence: Hi [one arm wave, yellow light, joy eyes], 
    ...
    print("=== STARTING CLOSING DIRECT BEHAVIOR ===")
    ...
    if os.path.exists(audio_file):
        ...
        misty.save_audio("closing_direct.mp3", convert_to_byte(audio_file), False, True)
        
        # Initial setup - Joy expression and yellow light
        ...
        misty.move_arms(90, 5, 50)  # Right arm higher for "Hi" (30 instead of 45)
        
        # Play audio with their exact approach
        feedback1 = misty.play_audio(fileName="closing_direct.mp3", volume=80)
        ...
        # "Would you like to learn more about our booth" - hands down, tilt head left
        misty.move_arms(90, 90, 50)  # Arms down
        misty.move_head(pitch=0, roll=-25, yaw=0)  # Head TILT left (roll, not yaw)
        time.sleep(3)  # Time for final phrase
        
        # Return to neutral
        misty.move_head(pitch=0, roll=0, yaw=0)
        
        return True, "✨ Direct closing with exact working pattern completed"
    else:
        return False, f"❌ Direct.mp3 file not found in {os.path.dirname(__file__)}"

if __name__ == "__main__":
    execute_closing_direct()