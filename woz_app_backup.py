...
def reset_misty_body():
    ...
    try:
        misty = Robot(ROBOT_IP)
        # Reset both arms to down position (0 degrees)
        response = misty.move_arms(0, 0, 100)  # left_arm, right_arm, velocity
        if response.status_code == 200:
            return True, "🔄 Arms reset to down position"
        else:
            return False, f"❌ Reset failed. Status: {response.status_code}"
    except Exception as e:
        return False, f"❌ Reset error: {str(e)}"

def raise_misty_arms():
    """Raise Misty's arms for testing body reset functionality"""
    try:
        misty = Robot(ROBOT_IP)
...
def main():
    ...
    if st.sidebar.button("🔄 Body Reset", help="Reset Misty's body to normal position"):
        success, message = raise_misty_arms()
        if success:
